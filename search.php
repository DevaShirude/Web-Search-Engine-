...
if ($query)
{
  ...
  $solr = new Apache_Solr_Service('localhost', 8983, '/solr/MySearch/');
  if( ! $solr->ping()) { 
            echo 'Sorry! Solr service is not available'; 
  } 
  else{
...
<html>
  ...
  <body style= "background-color:#F98268;">
     <h1 align="center" style="font-family: 'Verdana'; margin-top: 30px;"> Solr Search Engine with advanced capabilties </h1><br/>
    <form accept-charset="utf-8" method="get" id="searchform" align="center">
      Search : <input id="q" name="q" type="text" value="<?php echo htmlspecialchars($query, ENT_QUOTES, 'utf-8'); ?>" list="searchresults" placeholder="Enter your query here" autocomplete="off"/>
  ...
  <input type="radio" name="search" <?php if (isset($_GET['search']) && $_GET['search']=="pagerank") echo 'checked="checked"';?> value="pagerank" /> PageRank <br><br>
    ...
    <script>
   $(function() {
     var URL_PREFIX = "http://localhost:8983/solr/MySearch/suggest?q=";
...
$csvArray =  array_map('str_getcsv', file('URLtoHTML_latimes.csv'));
$count =0;
$pre="";
// display results
if ($results)
...
<?php
  foreach ($results->response->docs as $doc)
  {
    $id = $doc->id;
    $or_id = $id;
    $id = str_replace("/Users/devashreeshirude/Desktop/crawl_data/latimes/","",$id);
    ...
    $html_to_text_files_dir = "/Users/devashreeshirude/Desktop/crawl_data/latimes/";///////
...
<li>
        <table style="border: 0.75px solid black; width=700px text-align: left">
          <tr>
          <?php
          if(sizeof($title)>1){
          ?>
            <qw><?php echo "<a href = '{$link}' STYLE='text-decoration:none'><font size='4px' font-family='Verdana'><b>".implode(" ",$title)."</b></font></a>" ?></qw><?php } else { ?>
        
          <qw><?php echo "<a href = '{$link}' STYLE='text-decoration:none'><font size='4px' font-family= 'Verdana'><b>".$title."</b></font></a>" ?></qw><?php } ?>
          </tr>
          <tr>
            <th><?php echo htmlspecialchars("link", ENT_NOQUOTES, 'utf-8'); ?></th>
            <td><?php echo "<a href = '{$link}' STYLE='text-decoration:none' 'font-family:Verdana'><st>".$link."</st></a>" ?></td>
          </tr>
          <tr>
